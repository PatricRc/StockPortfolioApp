...
if 'transactions' not in st.session_state:
    st.session_state.transactions = pd.DataFrame(columns=["Symbol", "Date", "Shares", "Price", "Current Price"])
...
with st.form(key="add_transaction"):
    ...
    input_price = st.number_input("Purchase Price", min_value = 0.01)
    ...
    if submitted:
        ...
        if current_price is not None:
          new_transaction = pd.DataFrame({"Symbol": [symbol], "Date": [date], "Shares": [shares], "Price": [current_price], "Current Price": [current_price]})
